...
impl<P> Renderer<P> {
    ...
    pub fn new(name: &str, w: u32, h: u32) -> Self {
        ...
        let (event_tx, event_rx): (mpsc::Sender<events::Event>, mpsc::Receiver<events::Event>) =
            mpsc::channel();
        ...
        for event in sdl_events.poll_iter() {
            match event {
                ...
                SdlEvent::KeyDown {
                    ...
                    self.event_tx.send(Event::Up).unwrap();
                }
                SdlEvent::KeyDown {
                    keycode: Some(Keycode::Down),
                    ..
                } => {
                    self.event_tx.send(Event::Down).unwrap();
                }
                SdlEvent::KeyDown {
                    keycode: Some(Keycode::Left),
                    ..
                } => {
                    self.event_tx.send(Event::Left).unwrap();
                }
                SdlEvent::KeyDown {
                    keycode: Some(Keycode::Right),
                    ..
                } => {
                    self.event_tx.send(Event::Right).unwrap();
                }
                SdlEvent::KeyDown {
                    keycode: Some(Keycode::Return),
                    ..
                } => {
                    self.event_tx.send(Event::Select).unwrap();
                }
                SdlEvent::KeyDown {
                    keycode: Some(Keycode::Backspace),
                    ..
                } => {
                    self.event_tx.send(Event::Back).unwrap();
                }
                _ => {}
            }
        }
    ...
    fn render_common(&mut self, data: &[u8]) {
        ...
        let mut texture = creator
            .create_texture_target(PixelFormatEnum::RGBA8888, self.w, self.h)
            .unwrap();
...
mod tests {
    ...
    fn test_bw_to_rgba32() {
        let pixels = [0b1000_0001, 0b0100_0010];
        let expected = [
    ...
    fn test_rgb24_to_rgba32() {
        ...
        let expected = [
            ...
            0xff, 0x33, 0x22, 0x11, 0xff, 0x66, 0x55, 0x44, 0xff, 0x99, 0x88, 0x77, 0xff, 0xcc,
            0xbb, 0xaa,
        ]
        .to_vec();
